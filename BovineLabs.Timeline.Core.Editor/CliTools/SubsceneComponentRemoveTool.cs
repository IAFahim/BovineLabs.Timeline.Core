...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        ...
        Group = "vex",
        Description = "Remove a component (by type name) from a SubScene object. The replayable inverse of an ensure_component add; idempotent (no-op when the component is already absent).")]
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                using (var session = SubSceneSession.Open(p.OptString("subscene")))
                {
                    ...
                    var undo = new object[]
                    {
                        new { tool = "ensure_component", @params = new { subscene = session.SubscenePath, @object = objPath, component = compType.Name } },
                    };

                    if (comp == null)
                        return ToolEnvelope.Ok($"'{objPath}' has no {compType.Name} (already removed).",
                            result: new { removed = false }, undo: undo);

                    Object.DestroyImmediate(comp, true);
                    session.Save();

                    return ToolEnvelope.Ok($"Removed {compType.Name} from '{objPath}'.",
                        result: new { removed = true }, undo: undo);
                }
            }
