...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        Name = "director_bind",
        Group = "vex",
        Description = "Set a director's playableAsset and generic bindings (SetGenericBinding of a COMPONENT), with full PRE-capture so the returned undo restores the prior asset + binding table. §4 wiring + §6 UNDO-1.")]
    public static class DirectorBindTool
    {
        public class Parameters
        {
            ...
            [ToolParameter("Array of { track: name/index, object: hierarchyPath, component: typeName | \"GameObject\" }.")]
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                using (var session = SubSceneSession.Open(p.OptString("subscene")))
                {
                    ...
                    var pre = Capture.Director(d); // BEFORE any change

                    // Clear requested bindings against the CURRENT asset, BEFORE any asset swap — so an
                    // asset-swap undo can clear the tracks it added (they still exist on the outgoing
                    // asset) instead of looking them up in the restored asset and missing them, which
                    // would strand a {fileID:0} dangling binding key.
                    ...
                    // playableAsset: present (maybe null) vs omitted
                    ...
                    {
                        if (timeline == null)
                            return ToolEnvelope.Error("MISSING_PREREQUISITE", "Director has no TimelineAsset to bind tracks on (set 'asset' first).");

                        foreach (var bt in bindings)
                        {
                            if (!(bt is JObject bo))
                                return ToolEnvelope.Error("BAD_VALUE", "Each binding must be an object {track, object, component}.");

                            string trackSel = bo["track"]?.ToString();
                            var track = TimelineReflect.FindTrack(timeline, trackSel);
                            if (track == null) return ToolEnvelope.Error("NOT_FOUND", $"No track '{trackSel}' in the current asset.");

                            string objPath = bo["object"]?.ToString();
                            ...
                            string compName = bo["component"]?.ToString();
                            UnityEngine.Object bindObj;
                            ...
                            else
                            {
                                ...
                                if (comp == null) return ToolEnvelope.Error("NOT_FOUND", $"'{objPath}' has no component '{compName}'.");
                                bindObj = comp;
                            }
                    ...
                    // Build the inverse: restore prior playableAsset + each prior binding; clear tracks we newly bound.
                    var undoBindings = new List<object>();
                    foreach (var b in pre.bindings)
                        if (b.boundPath != null)
                            undoBindings.Add(new { track = b.trackName, @object = b.boundPath, component = b.boundComponentType });
                    ...
                    var undo = new object[]
                    {
                        new { tool = "director_bind", @params = new
                        {
                    ...
                    return ToolEnvelope.Ok(
                        ...
                        result: new { director = pre.path, playableAsset = post.playableAsset, bindings = post.bindings },
                        pre: pre,
                        undo: undo);
                }
            }
