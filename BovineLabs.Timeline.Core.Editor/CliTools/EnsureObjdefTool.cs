    ...
    public static class EnsureObjdefTool
    {
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                var defPath = p.RequireString("definition");
                var prefabPath = p.RequireString("prefab");
                var friendlyName = p.OptString("friendly_name");
                ...
                if (state.Satisfied)
                    return EnsureResult.Satisfied($"ObjectDefinition '{defPath}' ok (id {state.Id}, back-link OK).",
                        target, state.Before);

                if (dryRun)
                    return EnsureResult.WouldFixResult($"Would repair ObjectDefinition '{defPath}'.", target,
                        state.Before);
                ...
                if (!state.Exists)
                {
                    ...
                    {
                        tool = "asset_delete",
                        @params = new { asset = defPath, folder_if_empty = pre.folderExisted ? null : pre.folder }
                    });
                }
                ...
                if (!state.BackLinkOk)
                {
                    var resp = PrefabObjdefLinkTool.HandleCommand(new JObject
                        { ["prefab"] = prefabPath, ["definition"] = defPath });
                    if (Responses.IsError(resp))
                    {
                        RollbackPartial(undo);
                        return resp;
                    }
                ...
                var note = after.Id > 0 ? $"id {after.Id}" : "id still pending (postprocessor)";
