...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        ...
        Group = "vex",
        Description =
            "Author a whole track end-to-end in one call: timeline_create -> clip_add(xN) -> director_bind -> exposed_ref_wire(opt) -> timeline_verify, returning a memory-card result with the combined reverse-ordered undo. Rolls back on partial failure.")]
    public static class TrackAuthorTool
    {
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                var director = p.RequireString("director");
                var asset = p.RequireString("asset");
                var trackType = p.RequireString("track_type");
                ...
                var verify = p.OptBool("verify", true);
                var overwrite = p.OptBool("overwrite", false);

                var realTrackName = string.IsNullOrEmpty(trackName)
                ...
                using (var s = SubSceneSession.Open(subscene))
                {
                    ...
                    if (go == null)
                        return ToolEnvelope.Error("NOT_FOUND", $"No object '{director}' in {s.SubscenePath}.");
                ...
                if (clips != null)
                    foreach (var ct in clips)
                    {
                        ...
                        var cp = new JObject
                            { ["asset"] = asset, ["track"] = realTrackName, ["clip_type"] = co["clip_type"] };
                        foreach (var key in new[]
                                     { "start", "duration", "display_name", "blend_in", "blend_out", "fields" })
                            if (co[key] != null)
                                cp[key] = co[key];
                ...
                return ToolEnvelope.Ok(
                    ...
                    new
                    {
                        created = new { assetPath = asset, trackName = realTrackName }, director, verify = verifyData
                    },
                    new { director = dirPre },
                    undo.ToArray());
            }
        ...
        private static object Fail(string step, object resp, List<object> undo)
        {
            ...
            var msg = resp is ErrorResponse er ? er.message : "failed";
        ...
        public class Parameters
        {
            ...
            [ToolParameter(
                "Array of clip_add specs: { clip_type, start, duration, blend_in, blend_out, display_name, fields }.")]
            ...
            [ToolParameter(
                "Allow replacing an existing asset at the path (default false). When false, an existing asset fails the call before anything is mutated, so a hand-built timeline is never clobbered.")]
