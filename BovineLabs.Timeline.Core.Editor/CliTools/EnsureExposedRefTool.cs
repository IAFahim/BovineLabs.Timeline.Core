...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        ...
        Group = "vex",
        Description = "Idempotent: ensure a clip's ExposedReference<T> field resolves to a SubScene object via the director. already-ok when it already points there; otherwise wires it via exposed_ref_wire (passing its two-sided undo through). dry_run reports without mutating.")]
    public static class EnsureExposedRefTool
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                string targetPath = p.RequireString("target");
                bool dryRun = p.OptBool("dry_run", false);
                var target = new { director = directorSel, track = trackSel, clip = clipSel, field, target = targetPath };
                ...
                using (var session = SubSceneSession.Open(p.OptString("subscene")))
                {
                    ...
                    {
                        var wired = d.GetReferenceValue(exposed, out bool valid);
                ...
                if (satisfied) return EnsureResult.Satisfied($"{clipSel}.{field} already wired to {targetPath}.", target, before);
                if (dryRun) return EnsureResult.WouldFixResult($"Would wire {clipSel}.{field} -> {targetPath}.", target, before);
                ...
                return EnsureResult.Applied($"Wired {clipSel}.{field} -> {targetPath}.",
                    target, before, after: new { wiredPath = targetPath }, undo: Responses.Undo(resp));
            }
