...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        ...
        Group = "vex",
        Description =
            "Idempotent: ensure a .playable holds a track of the given type and name. already-ok when the asset+track already exist; otherwise creates them via timeline_create (and passes its asset_delete undo through). dry_run reports without mutating. The L1 setup block behind every clip.")]
    public static class EnsureTrackTool
    {
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                var trackName = p.OptString("track_name", trackType.Name);
                ...
                if (dryRun)
                    return EnsureResult.WouldFixResult($"Would create {trackName} ({trackType.Name}) in {assetPath}.",
                        target, before);
                ...
                return EnsureResult.Applied(
                    ...
                    target, before, new { assetExisted = true },
                    Responses.Undo(resp));
            }
