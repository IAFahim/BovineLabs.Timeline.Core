    ...
    public static class MechanicDoctorTool
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                {
                    // The doctor only exercises idempotent ensures — the core clip mutation is not a
                    // health check and is skipped.
                    ...
                    object resp;
                    try { resp = handler.Invoke(null, new object[] { args }); }
                    catch (System.Exception ex)
                    {
                        failed++;
                        findings.Add(new { req.Label, tool = req.Tool, satisfied = false, action = "error", error = ex.Message });
                        continue;
                    }
                    ...
                    {
                        failed++;
                        findings.Add(new { req.Label, tool = req.Tool, satisfied = false, action = "error", error = Responses.Message(resp) });
                        continue;
                    }

                    var resultObj = Responses.Section(resp, "result");
                    var jr = resultObj != null ? JObject.FromObject(resultObj) : null;
                    bool satisfied = jr?["satisfied"]?.Value<bool>() ?? false;
                    string action = jr?["action"]?.ToString() ?? "unknown";
                    ...
                    findings.Add(new { req.Label, tool = req.Tool, satisfied, action, message = Responses.Message(resp) });
                }

                journal.Reverse();
                string verb = fix ? $"{fixedCount} fixed, {ok} already-ok" : $"{unmet} would-fix, {ok} ok";
                if (failed > 0) verb += $", {failed} failed";
                return ToolEnvelope.Ok(
                    $"mechanic_doctor ({(fix ? "fix" : "check")}): {verb}.",
                    result: new { mode = fix ? "fix" : "check", ok, unmet, @fixed = fixedCount, failed, findings },
