...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        ...
        Group = "vex",
        Description =
            "Author a whole mechanic from a few keys: resolve its requirement manifest (reflectable from the clip/track types + the package's hand-declared traps), run each idempotent ensure_* in phase order, add the core clip, verify, and roll back the whole journal on any failure. The generic L2 engine — pass kind=trap (or explicit clip_type/track_type) plus source/prefab/objdef.")]
    public static class MechanicAuthorTool
    {
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                foreach (var req in reqs)
                {
                    ...
                    if (handler == null)
                        return Fail(req.Label,
                            ToolEnvelope.Error("NOT_FOUND", $"No tool '{req.Tool}' for requirement '{req.Label}'."),
                            journal);

                    object resp;
                    try
                    {
                        resp = handler.Invoke(null, new object[] { req.Params });
                    }
                    catch (Exception ex)
                    {
                        return Fail(req.Label, ToolEnvelope.Error("RECIPE_FAILED", $"{req.Tool} threw: {ex.Message}"),
                            journal);
                    }
                    ...
                    steps.Add(new
                    {
                        phase = req.Phase.ToString(), tool = req.Tool, label = req.Label,
                ...
                {
                    var vp = new JObject
                        { ["asset"] = ctx.Asset, ["subscene"] = ctx.Subscene, ["director"] = ctx.Director };
                ...
                return ToolEnvelope.Ok(
                    ...
                    new { mechanic = kind ?? ctx.ClipType.Name, director = ctx.Director, asset = ctx.Asset, steps },
        ...
        private static object Fail(string label, object resp, List<object> journal)
        {
            ...
            var msg = Responses.Message(resp) ?? "failed";
