...
namespace BovineLabs.Timeline.Core.Editor.CliTools
{
    [UnityCliTool(
        Name = "subscene_dump",
        Group = "vex",
        Description = "Field-level dump of a SubScene: every root -> object -> component -> serialized field values. Read-only; auto-detects the subscene and restores the parent scene.")]
    public static class SubsceneDumpTool
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                string objectPath = p.OptString("object");
                string compFilter = p.OptString("component");
                bool fields = p.OptBool("fields", true);
                int maxDepth = p.OptInt("depth", -1);
                ...
                using (var session = SubSceneSession.Open(subscene))
                {
                    ...
                    if (!string.IsNullOrEmpty(objectPath))
                    {
                        ...
                        if (go == null)
                            return ToolEnvelope.Error("NOT_FOUND", $"No object '{objectPath}' in {session.SubscenePath}.");
                    ...
                    return ToolEnvelope.Ok(
                        ...
                        result: new { subscene = session.SubscenePath, roots });
                }
            }
            catch (ToolException e) { return ToolEnvelope.FromException(e); }
        }

        private static object DumpGo(Transform t, int depth, int maxDepth, bool fields, string compFilter, ref int compCount)
        {
            var go = t.gameObject;
            ...
            foreach (var c in go.GetComponents<Component>())
            {
                ...
                {
                    JObject f;
                    try { f = TimelineReflect.ReadSerializedFields(c); }
                    catch { f = null; }
